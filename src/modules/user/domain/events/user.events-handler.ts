...
import { UserCreateEventHandler } from '../services/user-create.event-handler';
import { Inject } from '@nestjs/common';
import { EmailService } from '../../../mail/email.service';

// command handler 가 이벤트를 발생하고 publish 하면, 이벤트의 내용을 처리해야하므로 이 핸들러가 처리를 함.

@EventsHandler(UserCreateEventHandler)
export class UserEventsHandler implements IEventHandler<UserCreatedEvent> {
  constructor(@Inject(EmailService) readonly emailService: EmailService) {}
